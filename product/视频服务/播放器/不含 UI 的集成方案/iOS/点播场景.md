...
1. 下载 [最新版本](https://liteav.sdk.qcloud.com/download/latest/TXLiteAVSDK_Player_iOS_latest.zip) 的 SDK + Demo 开发包。
...
auth.sign = @"psignxxxxx"; // 如果是加密视频，必须填写
...
::: URL 方式
只需要传入下载地址即可，不支持嵌套 HLS 格式，仅支持单码流的HLS下载。
...
p.sign = @"psignxxxxx"; // 如果是加密视频，必须填写
[_txVodPlayer startVodPlayWithParams:p];
```
